export async function registerCurrentTranscriptSource({ characterInstanceId, avatarUrl, chatLocator, operatorActionId, recordedAt = new Date().toISOString(), fetchImpl = globalThis.fetch } = {}) {
    if (![characterInstanceId, avatarUrl, chatLocator, operatorActionId].every((value) => typeof value === 'string' && value.trim()) || typeof fetchImpl !== 'function') return Object.freeze({ state: 'REFUSED', reason: 'SOURCE_REGISTRATION_INPUT_INVALID' });
    try {
