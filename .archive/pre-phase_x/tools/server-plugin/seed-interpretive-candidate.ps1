...
Choose one:
  - isolated smoke reset first:
      powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1" -HostName $HostName -Port $Port -ResetFirst
  - or intentionally allow reuse of the dirty default line:
      powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1" -HostName $HostName -Port $Port -AllowDirtyDefaultLine
