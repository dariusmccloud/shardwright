...
- Legacy helper scripts remain available only for fixture seeding and lower-level proof work:
  ...
  - proof/reference script: `.archive/pre-phase_x/tools/server-plugin/prove-c0-6-4-2.ps1`
  - candidate helper: `.archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1`
  - smoke reset helper: `.archive/pre-phase_x/tools/server-plugin/reset-interpretive-smoke-storage.ps1`
...
- For a clean smoke run on the default Jeep line, seed with:
  - `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1" -HostName "<HostName>" -Port <Port> -ResetFirst -RestartHostAfterReset`
...
- bundled proof entry point: `.archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c.ps1`
- fresh-install proof entry point: `.archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c-fresh-install.ps1`
- corrected-child proof entry point: `.archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c-corrected-child.ps1`
- targeted commands:
  ...
  - `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c-fresh-install.ps1" -HostName "SillyTavern" -Port 8000`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c-corrected-child.ps1" -HostName "SillyTavern" -Port 8000`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-7c-rollback-recovery.ps1" -HostName "SillyTavern" -Port 8000`
...
- [x] Confirm architectural warm archive remains blocked on every relevant surface, not only review
- [x] Confirm no architectural output enters warm RAG storage through any alternate path
- [x] Historical C0.8.0C baseline: confirm pre-contract Architectural retrieval remained unavailable and failed closed rather than partially degrading (superseded for governed discovery by C0.8.0C.1-C0.8.0C.3)

### Architectural discovery RAG

- [ ] Rebuild/index only persisted, provenance-complete Architectural shards
- [ ] Retrieve section-aware Architectural evidence with exact source labels and hashes
- [ ] Exclude stale, malformed, missing-provenance, and mixed-profile results
- [ ] Preserve Narrative RAG behavior and collection isolation
- [ ] Report backend or re-ranker degradation without presenting retrieval as complete
- [ ] Confirm retrieval performs no authority-changing operation
- [ ] Repeat the supported proof in packaged SillyTavern and SillyBunny hosts
...
- Current boundary:
  ...
  - helper script: `.archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1`
...
- [x] Saved Architectural-shard evidence feeds governed proposal generation through the host Sharder save path, opens the exact admitted Review revision, and preserves the saved shard on governed proposal refusal (`C0.6.9` closeout proof)
...
- contract: `.archive/pre-phase_x/contracts/pre-phase_x/PHASE_C0_6_4_5_PUBLICATION_POLICY_BOOTSTRAP_AND_GUIDED_OPERATOR_FLOW_BRIEF.md`
- execution order: `.archive/pre-phase_x/contracts/pre-phase_x/C0_6_4_5_CLOSEOUT_AND_NEXT_LIFT_PLAN.md`
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-4-5a.ps1" -HostName "SillyTavern" -Port 8000`
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-4-5b.ps1" -HostName "SillyTavern" -Port 8000`
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File ".archive/pre-phase_x/tools/server-plugin/prove-c0-6-4-5c.ps1" -HostName "SillyTavern" -Port 8000`
...
- [ ] If no host UI entry point exists, seed one governed candidate via `.archive/pre-phase_x/tools/server-plugin/seed-interpretive-candidate.ps1`
