...
> Maintenance note (2026-07-13): A stale route-test expectation left behind when commit `d2fe53f` narrowed the deterministic Jeep statement has been corrected. The narrowed statement proves `STRONGLY_SUPPORTED` / `SUPPORTED`; a separate explicit legacy-broad statement preserves `CONTRARY_EVIDENCE_PRESENT` / `TOO_BROAD`. See `C0_8_0A_1_GROUNDING_ASSERTION_MAINTENANCE_REPORT.md`.
