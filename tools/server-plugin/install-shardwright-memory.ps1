...
$source = Join-Path $PSScriptRoot 'shardwright-memory'
$packager = Join-Path $PSScriptRoot 'package-shardwright-memory.mjs'
$payloadManifestPath = Join-Path $source 'payload-manifest.json'

node $packager | Out-Host
if ($LASTEXITCODE -ne 0) {
    throw "Packaging shardwright-memory failed with exit code $LASTEXITCODE"
}
...
if ([string]$payloadManifest.pluginId -ne 'shardwright-memory') {
    throw "Refusing to install payload with non-canonical plugin ID: $($payloadManifest.pluginId)"
}
...
foreach ($hostRoot in $HostRoots) {
    ...
    $target = Join-Path $pluginsRoot 'shardwright-memory'
    if (Test-Path $target) {
