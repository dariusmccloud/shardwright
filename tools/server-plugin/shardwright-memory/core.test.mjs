...
test('legacy metadata is replay-readable through non-enumerable canonical aliases', () => {
    const legacyJsonl = [
        JSON.stringify({ chat_metadata: { summary_sharder: { messageIdentity: { status: 'IDENTITY_COMPLETE' } } } }),
        JSON.stringify({ extra: { summary_sharder: { messageIdentity: {
    ...
    const { records, invalidLines } = parseJsonlRecords(legacyJsonl);
    const summary = summarizePersistedChatMetadata(records, invalidLines);

    assert.equal(summary.chatIdentityStatus.status, 'IDENTITY_COMPLETE');
    assert.equal(summary.identity.presentCount, 1);
    assert.equal(records[0].chat_metadata.shardwright, records[0].chat_metadata.summary_sharder);
    assert.equal(records[1].extra.shardwright, records[1].extra.summary_sharder);
    assert.equal(Object.keys(records[0].chat_metadata).includes('shardwright'), false);
    assert.equal(JSON.stringify(records[1]).includes('"shardwright"'), false);
});

function makeTempRoot() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'shardwright-memory-'));
}
...
test('server storage identity copies verifies activates and retains legacy authority', () => {
    const root = makeTempRoot();
    const legacyRoot = path.join(root, 'summary-sharder');
    fs.mkdirSync(legacyRoot, { recursive: true });
    fs.writeFileSync(path.join(legacyRoot, 'interpretive-governance-ledger.jsonl'), '{"event":"legacy"}\n', 'utf8');

    const paths = getStoragePaths(root);
    const markerPath = path.join(paths.storageRoot, '.shardwright-storage-migration.json');
    const marker = JSON.parse(fs.readFileSync(markerPath, 'utf8'));

    assert.equal(path.basename(paths.storageRoot), 'shardwright');
    assert.equal(marker.status, 'ACTIVE');
    assert.equal(marker.sourceStorageRoot, 'summary-sharder');
    assert.equal(fs.readFileSync(paths.interpretiveGovernanceLedgerPath, 'utf8'), '{"event":"legacy"}\n');
    assert.equal(fs.readFileSync(path.join(legacyRoot, 'interpretive-governance-ledger.jsonl'), 'utf8'), '{"event":"legacy"}\n');
    assert.equal(getStoragePaths(root).storageRoot, paths.storageRoot);
});

test('server storage migration replays a verified legacy operational database from a distinct canonical copy', () => {
    const root = makeTempRoot();
    const legacyPaths = getLegacyStoragePaths(root);
    const legacyAdapter = openOperationalDatabase(legacyPaths, { now: 1000 });
    snapshotOperationalDatabase(legacyAdapter, legacyPaths);
    const legacyManifest = loadManifest(legacyAdapter);
    legacyAdapter.close();

    const canonicalPaths = getStoragePaths(root);
    const canonicalAdapter = openOperationalDatabase(canonicalPaths, { now: 2000 });
    const canonicalManifest = loadManifest(canonicalAdapter);
    canonicalAdapter.close();

    assert.notEqual(canonicalPaths.dbPath, legacyPaths.dbPath);
    assert.equal(fs.existsSync(legacyPaths.dbPath), true);
    assert.equal(fs.existsSync(canonicalPaths.dbPath), true);
    assert.deepEqual(canonicalManifest, legacyManifest);

    const markerPath = path.join(canonicalPaths.storageRoot, '.shardwright-storage-migration.json');
    const marker = JSON.parse(fs.readFileSync(markerPath, 'utf8'));
    marker.status = 'VERIFIED';
    fs.writeFileSync(markerPath, `${JSON.stringify(marker, null, 2)}\n`, 'utf8');
    assert.equal(getStoragePaths(root).storageRoot, canonicalPaths.storageRoot);
    assert.equal(JSON.parse(fs.readFileSync(markerPath, 'utf8')).status, 'ACTIVE');
});

test('server storage identity refuses unattributed canonical coexistence and later legacy drift', () => {
    const conflictRoot = makeTempRoot();
    fs.mkdirSync(path.join(conflictRoot, 'summary-sharder'), { recursive: true });
    fs.mkdirSync(path.join(conflictRoot, 'shardwright'), { recursive: true });
    assert.throws(() => getStoragePaths(conflictRoot), { code: 'SHARDWRIGHT_STORAGE_MIGRATION_CONFLICT' });

    const driftRoot = makeTempRoot();
    const legacyRoot = path.join(driftRoot, 'summary-sharder');
    fs.mkdirSync(legacyRoot, { recursive: true });
    fs.writeFileSync(path.join(legacyRoot, 'dnm-publication-ledger.jsonl'), '{"event":"before"}\n', 'utf8');
    getStoragePaths(driftRoot);
    fs.appendFileSync(path.join(legacyRoot, 'dnm-publication-ledger.jsonl'), '{"event":"after"}\n', 'utf8');
    assert.throws(() => getStoragePaths(driftRoot), { code: 'SHARDWRIGHT_STORAGE_MIGRATION_CONFLICT' });
});
