...
import {
    SUBJECT_SCOPED_POLICY_LEDGER_EVENT_TYPES,
    applySubjectScopedPolicyLedgerEvent,
    attestSubjectScopedProposalFacts,
    bindAssignedSubjectPolicyToSynthesisRequest,
    ...
    requireSubjectScopedEligibilityForAdmission,
...
function resolveManifestSourceMessages(messages, manifest) {
    ...
    for (const message of messages) {
        const messageId = String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim();
    ...
    if (selector.mode === 'contiguous_interval') {
        ...
        const startIndex = messages.findIndex((message) => (
            String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim() === startMessageId
        ));
        const endIndex = messages.findIndex((message) => (
            String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim() === endMessageId
        ));
...
const ALLOWED_EVIDENCE_INSPECTABILITY_STATES = new Set([
    'VERIFIED',
    'LEGACY_UNAVAILABLE',
]);
const ALLOWED_EVIDENCE_PREVIEW_KINDS = new Set([
    'MESSAGE_EXCERPT',
    'STRUCTURAL_FIELDS',
    'SHARD_EXCERPT',
]);
...
function normalizeEvidencePreview(preview, index, groundingLinks) {
    if (!preview || Array.isArray(preview) || typeof preview !== 'object') {
        throw createError(400, `evidencePreviews[${index}] must be an object`, 'ARCH_INVALID_PAYLOAD');
    }
    const basisType = normalizeEnumValue(
        preview.basisType,
        `evidencePreviews[${index}].basisType`,
        new Set(['SOURCE_OCCURRENCE', 'STRUCTURAL_RECORD']),
    );
    const basisRef = sanitizeIdentifier(preview.basisRef, `evidencePreviews[${index}].basisRef`);
    const matches = groundingLinks.filter((link) => (
        link.basisType === basisType
        && (basisType === 'SOURCE_OCCURRENCE' ? link.messageId : link.basisRecordId) === basisRef
    ));
    if (matches.length !== 1) {
        throw createError(
            409,
            `Evidence preview ${basisRef} must match exactly one frozen source occurrence`,
            matches.length === 0 ? 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING' : 'ARCH_EVIDENCE_PREVIEW_SOURCE_AMBIGUOUS',
            { basisType, basisRef, matchCount: matches.length },
        );
    }
    const groundingLink = matches[0];
    const previewKind = normalizeEnumValue(
        preview.previewKind,
        `evidencePreviews[${index}].previewKind`,
        ALLOWED_EVIDENCE_PREVIEW_KINDS,
    );
    const sourceLabel = String(preview.sourceLabel || '').trim();
    if (!sourceLabel) {
        throw createError(400, `evidencePreviews[${index}].sourceLabel is required`, 'ARCH_INVALID_PAYLOAD');
    }
    let sourceArtifactClass;
    let sourceRevisionIdentity;
    let previewContent;
    let messageRevisionHash = null;
    if (basisType === 'SOURCE_OCCURRENCE') {
        if (previewKind !== 'MESSAGE_EXCERPT') {
            throw createError(400, `Evidence preview ${basisRef} must use MESSAGE_EXCERPT`, 'ARCH_EVIDENCE_PREVIEW_KIND_MISMATCH');
        }
        const text = normalizePersistedText(preview?.previewContent?.text).trim();
        if (!text) {
            throw createError(409, `Evidence preview ${basisRef} has no readable source excerpt`, 'ARCH_EVIDENCE_PREVIEW_CONTENT_MISSING');
        }
        messageRevisionHash = String(preview.messageRevisionHash || preview?.sourceRevisionIdentity?.messageRevisionHash || '').trim();
        if (messageRevisionHash !== groundingLink.messageRevisionHash) {
            throw createError(
                409,
                `Evidence preview ${basisRef} does not match the frozen message revision`,
                'ARCH_EVIDENCE_PREVIEW_SOURCE_DRIFTED',
                { basisRef, expected: groundingLink.messageRevisionHash, observed: messageRevisionHash },
            );
        }
        sourceArtifactClass = 'MESSAGE';
        sourceRevisionIdentity = {
            chatInstanceId: groundingLink.chatInstanceId,
            messageId: groundingLink.messageId,
            messageRevisionHash,
        };
        previewContent = { text };
    } else {
        const recordVersion = Number(preview?.sourceRevisionIdentity?.recordVersion ?? preview.recordVersion);
        const recordHash = String(preview?.sourceRevisionIdentity?.recordHash || preview.recordHash || '').trim();
        if (recordVersion !== groundingLink.basisRecordVersion || recordHash !== groundingLink.basisRecordHash) {
            throw createError(
                409,
                `Evidence preview ${basisRef} does not match the frozen structural record revision`,
                'ARCH_EVIDENCE_PREVIEW_SOURCE_DRIFTED',
                {
                    basisRef,
                    expected: { recordVersion: groundingLink.basisRecordVersion, recordHash: groundingLink.basisRecordHash },
                    observed: { recordVersion, recordHash },
                },
            );
        }
        const requestedArtifactClass = String(preview.sourceArtifactClass || 'STRUCTURAL_RECORD').trim();
        if (requestedArtifactClass === 'STRUCTURAL_RECORD') {
            if (previewKind !== 'STRUCTURAL_FIELDS') {
                throw createError(400, `Evidence preview ${basisRef} must use STRUCTURAL_FIELDS`, 'ARCH_EVIDENCE_PREVIEW_KIND_MISMATCH');
            }
            const fields = Array.isArray(preview?.previewContent?.fields)
                ? preview.previewContent.fields.map((field, fieldIndex) => {
                    const label = String(field?.label || '').trim();
                    const value = normalizePersistedText(field?.value).trim();
                    if (!label || !value) {
                        throw createError(
                            409,
                            `Evidence preview ${basisRef} structural field ${fieldIndex} is unreadable`,
                            'ARCH_EVIDENCE_PREVIEW_CONTENT_MISSING',
                        );
                    }
                    return { label, value };
                })
                : [];
            if (fields.length === 0) {
                throw createError(409, `Evidence preview ${basisRef} has no readable structural fields`, 'ARCH_EVIDENCE_PREVIEW_CONTENT_MISSING');
            }
            sourceArtifactClass = 'STRUCTURAL_RECORD';
            sourceRevisionIdentity = { recordVersion, recordHash };
            previewContent = { fields };
        } else if (requestedArtifactClass === 'SAVED_SHARD') {
            if (previewKind !== 'SHARD_EXCERPT') {
                throw createError(400, `Evidence preview ${basisRef} must use SHARD_EXCERPT`, 'ARCH_EVIDENCE_PREVIEW_KIND_MISMATCH');
            }
            const shardArtifactId = sanitizeIdentifier(
                preview?.sourceRevisionIdentity?.shardArtifactId,
                `evidencePreviews[${index}].sourceRevisionIdentity.shardArtifactId`,
            );
            const shardRevisionHash = String(preview?.sourceRevisionIdentity?.shardRevisionHash || '').trim();
            if (!shardRevisionHash.startsWith('sha256:')) {
                throw createError(409, `Evidence preview ${basisRef} has invalid saved-shard identity`, 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING');
            }
            const text = normalizePersistedText(preview?.previewContent?.text).trim();
            const sectionLabel = String(preview?.previewContent?.sectionLabel || '').trim();
            const rangeStart = Number(preview?.previewContent?.sourceRange?.startIndex);
            const rangeEnd = Number(preview?.previewContent?.sourceRange?.endIndex);
            if (!text || !sectionLabel || !Number.isInteger(rangeStart) || !Number.isInteger(rangeEnd) || rangeStart < 0 || rangeEnd < rangeStart) {
                throw createError(409, `Evidence preview ${basisRef} has no readable saved-shard excerpt and range`, 'ARCH_EVIDENCE_PREVIEW_CONTENT_MISSING');
            }
            ...
            previewContent = {
                text,
                sectionLabel,
                sourceRange: { startIndex: rangeStart, endIndex: rangeEnd },
            };
        } else {
            throw createError(400, `Evidence preview ${basisRef} has invalid sourceArtifactClass`, 'ARCH_INVALID_PAYLOAD');
        }
    }
    const semanticPayload = {
        basisType,
        ...
        sourceRevisionIdentity,
        sourceLabel,
        speakerLabel: String(preview.speakerLabel || '').trim() || null,
        contextLabel: String(preview.contextLabel || '').trim() || null,
        previewKind,
        previewContent,
        ...(basisType === 'SOURCE_OCCURRENCE' ? { messageRevisionHash } : {}),
        verificationState: 'VERIFIED',
    };
    const previewContentHash = hashCanonical(previewContent).hash;
    const evidencePreviewId = preview.evidencePreviewId
        ? sanitizeIdentifier(preview.evidencePreviewId, `evidencePreviews[${index}].evidencePreviewId`)
        : `evprev_${hashCanonical(semanticPayload).hash.slice('sha256:'.length, 'sha256:'.length + 24)}`;
    return {
        evidencePreviewId,
        groundingLinkId: groundingLink.groundingLinkId,
        ...semanticPayload,
        previewContentHash,
    };
}

function normalizeEvidencePreviewsForEnvelopeHash(evidencePreviews) {
    return evidencePreviews
        .map((entry) => ({
            basisType: entry.basisType,
            sourceArtifactClass: entry.sourceArtifactClass,
            basisRef: entry.basisRef,
            ...
            previewContentHash: entry.previewContentHash,
            ...(entry.basisType === 'SOURCE_OCCURRENCE' ? { messageRevisionHash: entry.messageRevisionHash } : {}),
            verificationState: entry.verificationState,
        }))
        .sort((left, right) => stableStringify(left).localeCompare(stableStringify(right)));
}

function validateEvidencePreviewCoverage(evidenceFindings, evidencePreviews) {
    const previewByBasisRef = new Map();
    for (const preview of evidencePreviews) {
        const matches = previewByBasisRef.get(preview.basisRef) || [];
        matches.push(preview);
        previewByBasisRef.set(preview.basisRef, matches);
    }
    for (const finding of evidenceFindings) {
        for (const basisRef of finding.basisRefs) {
            const matches = previewByBasisRef.get(basisRef) || [];
            if (matches.length !== 1) {
                throw createError(
                    409,
                    `Finding ${finding.findingId} requires one inspectable preview for ${basisRef}`,
                    matches.length === 0 ? 'ARCH_EVIDENCE_PREVIEW_REQUIRED' : 'ARCH_EVIDENCE_PREVIEW_AMBIGUOUS',
                    { findingId: finding.findingId, basisRef, matchCount: matches.length },
                );
            }
        }
    }
...
function buildReviewEnvelopeHash(proposalContentHash, groundingLinks, evidenceFindings, evidencePreviews, groundingOutcome, risk, policy, obligations) {
    return hashCanonical({
        proposalContentHash,
        ...
        evidencePreviews: normalizeEvidencePreviewsForEnvelopeHash(evidencePreviews),
        groundingOutcome,
        riskClass: risk.riskClass,
        riskReasons: risk.riskReasons,
...
function hasVerifiedEvidenceEnvelope(interpretation) {
    return interpretation?.evidenceEnvelopeVersion === EVIDENCE_ENVELOPE_SCHEMA_VERSION
        && interpretation?.evidenceInspectabilityState === 'VERIFIED'
        && Array.isArray(interpretation?.evidencePreviews)
        && interpretation.evidencePreviews.length > 0;
}

function assertVerifiedEvidenceEnvelope(interpretation) {
    if (!hasVerifiedEvidenceEnvelope(interpretation)) {
        throw createError(
            409,
            'This older proposal has no inspectable evidence preview. Rebuild it from its available sources as a new revision.',
            'ARCH_EVIDENCE_ENVELOPE_NOT_VERIFIED',
        );
    }
}
...
export function prepareInterpretiveCandidate(payload, timestamp = nowTimestamp(payload?.now)) {
    ...
    const evidenceFindings = Array.isArray(payload?.evidenceFindings)
        ...
        : [];
    const evidenceEnvelopeVersion = payload?.evidenceEnvelopeVersion === undefined
        ? null
        : Number(payload.evidenceEnvelopeVersion);
    if (evidenceEnvelopeVersion !== null && evidenceEnvelopeVersion !== EVIDENCE_ENVELOPE_SCHEMA_VERSION) {
        throw createError(400, 'evidenceEnvelopeVersion is unsupported', 'ARCH_EVIDENCE_ENVELOPE_VERSION_UNSUPPORTED');
    }
    const evidencePreviews = evidenceEnvelopeVersion === EVIDENCE_ENVELOPE_SCHEMA_VERSION
        ? (Array.isArray(payload?.evidencePreviews)
            ? payload.evidencePreviews.map((entry, index) => normalizeEvidencePreview(entry, index, groundingLinks))
            : [])
        : [];
    if (evidenceEnvelopeVersion === EVIDENCE_ENVELOPE_SCHEMA_VERSION) {
        validateEvidencePreviewCoverage(evidenceFindings, evidencePreviews);
    }
    const evidenceInspectabilityState = evidenceEnvelopeVersion === EVIDENCE_ENVELOPE_SCHEMA_VERSION
        ? 'VERIFIED'
        : 'LEGACY_UNAVAILABLE';
    ...
    const input = {
        ...
        groundingLinks,
        evidenceFindings,
        evidenceEnvelopeVersion,
        evidencePreviews,
        evidenceInspectabilityState,
        proposalContentHash,
...
function createLedgerEvents(prepared, timestamp) {
    ...
    for (const preview of prepared.candidate.evidencePreviews || []) {
        events.push({
            eventId: createId('iglevent'),
            eventType: 'EVIDENCE_PREVIEW_RECORDED',
            occurredAt: timestamp,
            memoryScopeId: prepared.candidate.memoryScopeId,
            interpretationId: prepared.candidate.interpretationId,
            interpretationRevisionId: prepared.candidate.interpretationRevisionId,
            payload: cloneJson(preview),
        });
    }
    events.push({
        eventId: createId('iglevent'),
...
const INTERPRETIVE_PROJECTION_TABLES = Object.freeze([
    'subject_scoped_proposal_eligibility_evaluations',
    'subject_scoped_proposal_fact_attestations',
    'subject_scoped_proposal_policy_bindings',
    'subject_scoped_proposal_fact_declarations',
    'subject_scoped_proposal_acknowledgments',
    'subject_scoped_proposal_policy_assignments',
    'subject_scoped_proposal_policy_profiles',
    ...
    'interpretation_evidence_envelopes',
...
function buildCandidatePublicationOperatorState(adapter, interpretation, continuityTargetId) {
    ...
    if (!hasVerifiedEvidenceEnvelope(interpretation)) {
        qualificationReasons.push('EVIDENCE_ENVELOPE_NOT_VERIFIED');
    }
...
async function readPersistedArchitecturalShardContext(request, payload = {}) {
    ...
    const shardwrightMetadata = header?.chat_metadata?.shardwright;
    const binding = shardwrightMetadata?.architecturalMemoryBinding;
    ...
    const manifests = Array.isArray(shardwrightMetadata?.shardManifests) ? shardwrightMetadata.shardManifests : [];
    const messages = records.slice(1);
    const shardMessage = messages.find((message) => (
        String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim() === shardMessageId
    ));
    if (!shardMessage) {
...
function buildArchitecturalShardSourceManifestEntries(context, memorySubjectId) {
    ...
    for (const message of context.sourceMessages) {
        const messageIdentity = message?.extra?.shardwright?.messageIdentity;
        const speakerIdentity = message?.extra?.shardwright?.speakerIdentity;
...
function deriveInterpretiveProposalFromDecision(context, decision, memorySubjectId, createdByEntityId) {
    ...
    for (const sourceRef of sourceRefs) {
        ...
        const messageId = context.sourceMessages[sourceIndex]?.extra?.shardwright?.messageIdentity?.messageId;
        if (!messageId) {
            unresolvedSourceRefs.push(sourceRef);
            continue;
        }
...
function buildPersistedShardEvidencePreviews(context, decision, sourceManifestEntries, proposalOverride) {
    const citedBasisRefs = new Set(
        proposalOverride.evidenceFindings.flatMap((finding) => finding.basisRefs),
    );
    const sourceStart = Number.parseInt(context.manifest.sourceStartPositionAtCreation, 10);
    const sourceEnd = Number.parseInt(context.manifest.sourceEndPositionAtCreation, 10);
    const shardRevisionHash = String(
        context.shardMessage?.extra?.shardwright?.messageIdentity?.revisionHash || '',
    ).trim();
    if (!Number.isInteger(sourceStart) || !Number.isInteger(sourceEnd) || sourceEnd < sourceStart) {
        throw createError(409, 'Saved shard evidence preview has no frozen source range', 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING');
    }
    if (!shardRevisionHash.startsWith('sha256:')) {
        throw createError(409, 'Saved shard evidence preview has no frozen shard revision', 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING');
    }

    const previews = [];
    for (const entry of sourceManifestEntries) {
        const basisRef = entry.sourceClass === 'STRUCTURAL_RECORD' ? entry.basisRecordId : entry.messageId;
        if (!citedBasisRefs.has(basisRef)) {
            continue;
        }
        if (entry.sourceClass === 'STRUCTURAL_RECORD') {
            if (basisRef !== `decision:${decision.decisionId}` || !String(decision.raw || '').trim()) {
                throw createError(409, `Saved shard structural evidence ${basisRef} could not be previewed`, 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING');
            }
            previews.push({
                basisType: 'STRUCTURAL_RECORD',
                sourceArtifactClass: 'SAVED_SHARD',
                basisRef,
                sourceRevisionIdentity: {
                    recordVersion: entry.basisRecordVersion,
                    recordHash: entry.basisRecordHash,
                    shardArtifactId: context.shardMessageId,
                    shardRevisionHash,
                },
                sourceLabel: `Architectural shard decision ${decision.decisionId}`,
                speakerLabel: String(context.shardMessage?.name || '').trim() || null,
                contextLabel: `${context.chatLocator}, messages ${sourceStart}-${sourceEnd}`,
                previewKind: 'SHARD_EXCERPT',
                previewContent: {
                    text: String(decision.raw).trim(),
                    sectionLabel: 'DECISIONS',
                    sourceRange: { startIndex: sourceStart, endIndex: sourceEnd },
                },
            });
            continue;
        }
        const sourceMessage = context.sourceMessages.find((message) => (
            String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim() === entry.messageId
        ));
        if (!sourceMessage) {
            throw createError(409, `Source occurrence ${entry.messageId} could not be previewed`, 'ARCH_EVIDENCE_PREVIEW_SOURCE_MISSING');
        }
        const sourceText = normalizePersistedText(sourceMessage.mes).trim();
        if (!sourceText) {
            throw createError(409, `Source occurrence ${entry.messageId} has no readable excerpt`, 'ARCH_EVIDENCE_PREVIEW_CONTENT_MISSING');
        }
        previews.push({
            basisType: 'SOURCE_OCCURRENCE',
            basisRef: entry.messageId,
            sourceRevisionIdentity: {
                chatInstanceId: entry.chatInstanceId,
                messageId: entry.messageId,
                messageRevisionHash: entry.messageRevisionHash,
            },
            sourceLabel: `${String(sourceMessage.name || '').trim() || 'Source message'}, ${context.chatLocator}`,
            speakerLabel: String(sourceMessage.name || '').trim() || null,
            contextLabel: context.chatLocator,
            previewKind: 'MESSAGE_EXCERPT',
            previewContent: { text: sourceText },
        });
    }
    return previews;
}
...
export async function createInterpretiveProposalFromArchitecturalShard(request, payload = {}) {
    ...
    const evidencePreviews = buildPersistedShardEvidencePreviews(
        context,
        decision,
        sourceManifestEntries,
        proposalOverride,
    ...
    const subjectPolicyBinding = bindAssignedSubjectPolicyToSynthesisRequest(request, {
        synthesisRunId,
        subjectEntityId: memorySubjectId,
        jurisdictionScopeId: context.memoryScopeId,
        evidenceSetHash: runResult.synthesisRun.sourceManifestHash,
        now: timestamp,
    });
    if (subjectPolicyBinding.assigned) {
        const groundingBasisRefs = proposalOverride.proposedBasis
            .map((entry) => String(entry.basisRecordId || entry.messageId || '').trim())
            .filter(Boolean)
            .sort();
        const declarations = listSubjectScopedProposalFactDeclarations(request, {
            subjectEntityId: memorySubjectId,
            jurisdictionScopeId: context.memoryScopeId,
            evidenceSetHash: runResult.synthesisRun.sourceManifestHash,
        }).filter((entry) => (
            entry.proposalKind === 'DESIGN_COMMITMENT'
            && entry.policyHash === subjectPolicyBinding.binding.policyHash
        ));
        const declarationRecords = declarations.map((entry) => ({
            recordId: entry.declarationId,
            recordType: entry.factType === 'STABILITY'
        ...
        const acknowledgmentRecords = buildSubjectScopedAcknowledgmentFactRecords(
            listSubjectScopedProposalAcknowledgments(request, {
                subjectEntityId: memorySubjectId,
                jurisdictionScopeId: context.memoryScopeId,
                evidenceSetHash: runResult.synthesisRun.sourceManifestHash,
            }),
            {
                ...
                policyHash: subjectPolicyBinding.binding.policyHash,
            },
        );
        attestSubjectScopedProposalFacts(request, {
            bindingTargetType: 'SYNTHESIS_REQUEST',
            bindingTargetId: synthesisRunId,
            subjectEntityId: memorySubjectId,
            proposalKind: 'DESIGN_COMMITMENT',
            proposalTrack: 'ARCHITECTURAL_DECISION',
            records: [
                {
                    recordId: `decision-explicitness:${decision.decisionId}`,
                    recordType: 'EXPLICITNESS_VERIFICATION',
                    evidenceSetHash: runResult.synthesisRun.sourceManifestHash,
                    state: 'VERIFIED',
                    basisRefs: [`decision:${decision.decisionId}`],
                },
                {
                    recordId: `manifest-grounding:${synthesisRunId}`,
                    recordType: 'GROUNDING_VERIFICATION',
                    evidenceSetHash: runResult.synthesisRun.sourceManifestHash,
                    state: 'VERIFIED',
                    basisRefs: groundingBasisRefs,
                },
                ...declarationRecords,
                ...acknowledgmentRecords,
            ],
            now: timestamp,
        });
        evaluateBoundSubjectScopedSynthesisRequest(request, {
            bindingTargetId: synthesisRunId,
            now: timestamp,
        });
    }
    let generateResult;
    try {
        generateResult = executeInterpretiveSynthesisRun(request, synthesisRunId, {
            ...
            stubProposalOverride: buildArchitecturalProposalOverride({ proposalOverride }),
            now: timestamp,
        }, { evidencePreviews });
    } catch (error) {
        if (String(error?.code || '').startsWith('ARCH_SUBJECT_POLICY_')) {
            error.synthesisRunId = synthesisRunId;
        }
        throw error;
    }
    return {
...
function buildChildRevisionPayload(parentInterpretation, payload = {}, createdFromDispositionId, reviewerRole, timestamp) {
    ...
    return {
        ...
        evidenceFindings: revised.evidenceFindings || parentInterpretation.evidenceFindings,
        evidenceEnvelopeVersion: parentInterpretation.evidenceEnvelopeVersion,
        evidencePreviews: parentInterpretation.evidencePreviews,
        now: timestamp,
    };
}
...
function carryForwardPreparedReviewerApproval(prepared, reviewerRole, reviewerEntityId, timestamp) {
    ...
    const reviewEnvelopeHash = buildReviewEnvelopeHash(
        ...
        prepared.candidate.evidencePreviews || [],
...
function buildPreparedFromLedgerEvents(events) {
    ...
    const evidencePreviews = events
        .filter((entry) => entry.eventType === 'EVIDENCE_PREVIEW_RECORDED')
        .map((entry) => cloneJson(entry.payload))
        .sort((a, b) => String(a.evidencePreviewId).localeCompare(String(b.evidencePreviewId)));
    ...
    return {
        candidate: {
            ...
            groundingLinks,
            evidenceFindings,
            evidenceEnvelopeVersion: evidencePreviews.length > 0 ? EVIDENCE_ENVELOPE_SCHEMA_VERSION : null,
            evidencePreviews,
            evidenceInspectabilityState: evidencePreviews.length > 0 ? 'VERIFIED' : 'LEGACY_UNAVAILABLE',
...
export function replayInterpretiveLedger(request, options = {}) {
    ...
    const subjectScopedPolicyEvents = [];
    ...
    for (const event of ledgerEvents) {
        if (SUBJECT_SCOPED_POLICY_LEDGER_EVENT_TYPES.has(event.eventType)) {
            subjectScopedPolicyEvents.push(event);
            continue;
        }
    ...
    try {
        ...
        const subjectPolicyReplayRank = new Map([
            ['SUBJECT_SCOPED_POLICY_PROFILE_REGISTERED', 0],
            ['SUBJECT_SCOPED_POLICY_PROFILE_ASSIGNED', 1],
            ['SUBJECT_SCOPED_POLICY_FACT_DECLARED', 2],
            ['SUBJECT_SCOPED_POLICY_ACKNOWLEDGMENT_RECORDED', 3],
            ['SUBJECT_SCOPED_POLICY_PROFILE_BOUND', 4],
            ['SUBJECT_SCOPED_POLICY_FACTS_ATTESTED', 5],
            ['SUBJECT_SCOPED_POLICY_ELIGIBILITY_EVALUATED', 6],
        ]);
        for (const event of subjectScopedPolicyEvents.sort((a, b) => (
            (subjectPolicyReplayRank.get(a.eventType) ?? 99) - (subjectPolicyReplayRank.get(b.eventType) ?? 99)
            || Number(a.occurredAt) - Number(b.occurredAt)
            || String(a.eventId).localeCompare(String(b.eventId))
        ))) {
            applySubjectScopedPolicyLedgerEvent(adapter, event);
        }
...
function persistPreparedCandidateRows(adapter, prepared, timestamp) {
    ...
    if (prepared.candidate.evidenceEnvelopeVersion === EVIDENCE_ENVELOPE_SCHEMA_VERSION) {
        adapter.run(
            `INSERT INTO interpretation_evidence_envelopes (
                interpretation_revision_id, envelope_version, inspectability_state, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?)`,
            [
                prepared.candidate.interpretationRevisionId,
                prepared.candidate.evidenceEnvelopeVersion,
                prepared.candidate.evidenceInspectabilityState,
                timestamp,
                timestamp,
            ],
        );
        for (const preview of prepared.candidate.evidencePreviews) {
            adapter.run(
                `INSERT INTO interpretation_evidence_previews (
                    interpretation_revision_id, evidence_preview_id, grounding_link_id, basis_type,
                    basis_ref, source_artifact_class, source_revision_identity_json,
                    source_label, speaker_label, context_label, preview_kind,
                    preview_content_json, preview_content_hash, verification_state, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    prepared.candidate.interpretationRevisionId,
                    preview.evidencePreviewId,
                    preview.groundingLinkId,
                    preview.basisType,
                    preview.basisRef,
                    preview.sourceArtifactClass,
                    stableStringify(preview.sourceRevisionIdentity),
                    preview.sourceLabel,
                    preview.speakerLabel,
                    preview.contextLabel,
                    preview.previewKind,
                    stableStringify(preview.previewContent),
                    preview.previewContentHash,
                    preview.verificationState,
                    timestamp,
                ],
            );
        }
...
export function loadInterpretiveCandidateProjection(adapter, interpretationRevisionId) {
    ...
    const evidenceEnvelope = adapter.get(
        'SELECT * FROM interpretation_evidence_envelopes WHERE interpretation_revision_id = ?',
        [interpretationRevisionId],
    );
    const evidencePreviews = adapter.all(
        'SELECT * FROM interpretation_evidence_previews WHERE interpretation_revision_id = ? ORDER BY evidence_preview_id',
        [interpretationRevisionId],
    );
    ...
    return {
        ...
        evidenceEnvelopeVersion: evidenceEnvelope ? Number(evidenceEnvelope.envelope_version) : null,
        evidenceInspectabilityState: evidenceEnvelope
            && ALLOWED_EVIDENCE_INSPECTABILITY_STATES.has(String(evidenceEnvelope.inspectability_state || '').trim())
            ? evidenceEnvelope.inspectability_state
            : 'LEGACY_UNAVAILABLE',
        evidencePreviews: evidencePreviews.map((entry) => {
            const groundingLink = groundingLinks.find((link) => link.grounding_link_id === entry.grounding_link_id);
            const fallbackSourceRevisionIdentity = entry.basis_type === 'SOURCE_OCCURRENCE'
                ? {
                    chatInstanceId: groundingLink?.chat_instance_id || null,
                    messageId: groundingLink?.message_id || null,
                    messageRevisionHash: groundingLink?.message_revision_hash || null,
                }
                : {
                    recordVersion: groundingLink?.basis_record_version === null
                        ? null
                        : Number(groundingLink?.basis_record_version),
                    recordHash: groundingLink?.basis_record_hash || null,
                };
            const sourceArtifactClass = String(entry.source_artifact_class || '').trim()
                || (entry.basis_type === 'SOURCE_OCCURRENCE' ? 'MESSAGE' : 'STRUCTURAL_RECORD');
            const sourceRevisionIdentity = entry.source_revision_identity_json
                ? JSON.parse(entry.source_revision_identity_json)
                : fallbackSourceRevisionIdentity;
            return {
                evidencePreviewId: entry.evidence_preview_id,
                groundingLinkId: entry.grounding_link_id,
                basisType: entry.basis_type,
                sourceArtifactClass,
                basisRef: entry.basis_ref,
                sourceRevisionIdentity,
                sourceLabel: entry.source_label,
                speakerLabel: entry.speaker_label,
                contextLabel: entry.context_label,
                previewKind: entry.preview_kind,
                previewContent: JSON.parse(entry.preview_content_json),
                previewContentHash: entry.preview_content_hash,
                ...(entry.basis_type === 'SOURCE_OCCURRENCE'
                    ? { messageRevisionHash: groundingLink?.message_revision_hash || null }
                    : {}),
...
function evaluateInterpretivePublicationQualification(adapter, interpretation, policy, payload = {}, timestamp = Date.now()) {
    ...
    if (!hasVerifiedEvidenceEnvelope(interpretation)) {
        refusalCodes.push('EVIDENCE_ENVELOPE_NOT_VERIFIED');
    }
...
export function executeInterpretiveSynthesisRun(request, synthesisRunId, payload = {}, serverContext = {}) {
    const timestamp = nowTimestamp(payload?.now);
    const userRoot = getAuthenticatedUserRoot(request);
    const paths = getStoragePaths(userRoot);
    const adapter = openOperationalDatabase(paths, { now: timestamp });
    try {
        ...
        requireSubjectScopedEligibilityForAdmission(adapter, run.synthesisRunId);
        const candidatePayload = {
            ...
            ...(Array.isArray(serverContext.evidencePreviews) && serverContext.evidencePreviews.length > 0
                ? {
                    evidenceEnvelopeVersion: EVIDENCE_ENVELOPE_SCHEMA_VERSION,
                    evidencePreviews: cloneJson(serverContext.evidencePreviews),
                }
                : {}),
...
export function submitInterpretiveReviewDisposition(request, reviewRequestId, payload = {}) {
    ...
    try {
        ...
        if (['APPROVE', 'APPROVE_WITH_EDIT', 'APPROVE_FOR_SCOPE_ONLY'].includes(disposition)) {
            assertVerifiedEvidenceEnvelope(interpretation);
        }
