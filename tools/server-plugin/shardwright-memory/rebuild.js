...
function buildArtifactSemanticHash(outputMessage, manifest, metadata) {
    return sha256Text(stableStringify({
        outputMessageId: outputMessage?.extra?.shardwright?.messageIdentity?.messageId || null,
        outputRevisionHash: outputMessage?.extra?.shardwright?.messageIdentity?.revisionHash || null,
...
async function buildFrozenManifest(request, memoryScopeId, reconstructionRunId, requestKey, timestamp, candidatePaths) {
    ...
    for (const filePath of files) {
        ...
        const binding = header?.shardwright?.architecturalMemoryBinding || null;
        ...
        const shardManifests = Array.isArray(header?.shardwright?.shardManifests)
            ? header.shardwright.shardManifests
            : [];
        ...
        corpusFiles.push({
            ...
            identityStatus: String(header?.shardwright?.messageIdentity?.status || ''),
            frozenAt: timestamp,
        ...
        for (const manifestEntry of shardManifests) {
            ...
            const evidencePolicy = outputMessage?.extra?.shardwright?.evidencePolicy === 'exclude'
                ? 'exclude'
                : (outputMessage?.extra?.shardwright?.evidencePolicy ? String(outputMessage.extra.shardwright.evidencePolicy) : 'legacy-default-include');
            ...
            } else if (!outputMessage?.extra?.shardwright?.messageIdentity?.messageId) {
                admissionStatus = 'blocked';
            ...
            artifacts.push({
                sourceId,
                corpusFileId,
                artifactMessageId: outputMessage?.extra?.shardwright?.messageIdentity?.messageId || null,
                outputUid,
...
function buildTier2OccurrenceContext(manifest, corpus, message) {
    return {
        ...
        sourceMessageId: String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim(),
        sourceRevisionHash: String(message?.extra?.shardwright?.messageIdentity?.revisionHash || '').trim(),
        initFingerprint: String(message?.extra?.shardwright?.messageIdentity?.initFingerprint || '').trim(),
        message,
    };
}
...
function collectTier2Claims(manifest, corpusByFileId, admittedArtifactMessageIds) {
    ...
    for (const fileEntry of files) {
        ...
        for (const message of corpus.messages) {
            ...
            const evidencePolicy = String(message?.extra?.shardwright?.evidencePolicy || '').trim().toLowerCase();
...
async function compileCandidate(adapter, manifest, corpusByFileId) {
        ...
        if (fileEntry.chatInstanceId) {
            adapter.run(
                ...
                [
                    ...
                    '',
                    corpus.header?.shardwright?.architecturalMemoryBinding?.branchedFromChatInstanceId || null,
                    corpus.header?.shardwright?.architecturalMemoryBinding?.importedFromChatInstanceId || null,
                    timestamp,
                    timestamp,
                ],
            );
    ...
    for (const artifact of artifacts) {
        ...
        const normalizedManifest = normalizeShardManifest(
            artifact.corpus.header?.shardwright?.shardManifests?.find((entry) => String(entry?.manifestId || '') === String(artifact.sourceManifestId || ''))
        );
        ...
        if (coveredSourceMessageIds.length === 0
            ...
            coveredSourceMessageIds = artifact.corpus.messages
                ...
                .map((message) => String(message?.extra?.shardwright?.messageIdentity?.messageId || '').trim())
                .filter(Boolean);
        }
        ...
        for (const item of decisionItems) {
            ...
            const messageIdentity = outputMessage?.extra?.shardwright?.messageIdentity || {};
            const binding = artifact.corpus?.header?.shardwright?.architecturalMemoryBinding || {};
            ...
            const occurrence = {
                ...
                speakerEntityId: String(outputMessage?.extra?.shardwright?.speakerIdentity?.speakerEntityId || ''),
