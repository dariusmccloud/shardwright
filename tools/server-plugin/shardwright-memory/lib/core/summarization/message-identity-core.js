...
function ensureShardwrightRoot(record) {
    if (!record.extra || typeof record.extra !== 'object') {
        record.extra = {};
    }
    if (!record.extra.shardwright || typeof record.extra.shardwright !== 'object') {
        record.extra.shardwright = {};
    }
    return record.extra.shardwright;
}

function ensureShardwrightChatRoot(chatMetadata) {
    if (!chatMetadata.shardwright || typeof chatMetadata.shardwright !== 'object') {
        chatMetadata.shardwright = {};
    }
    return chatMetadata.shardwright;
}
...
export async function buildCorpusRevisionHash(messages, options = {}) {
    ...
    for (const message of messages || []) {
        const ss = message?.extra?.shardwright;
    ...
    return {
        ...
        entries: Array.isArray(messages)
            ? messages.map((message, index) => {
                const identity = message?.extra?.shardwright?.messageIdentity || {};
                return {
                    index,
...
export function reconcileMessageDeletionTombstones(previousSnapshot, currentMessages, chatMetadata, options = {}) {
    const ss = ensureShardwrightChatRoot(chatMetadata);
    ...
    const currentIds = new Set(
        (currentMessages || [])
            .map((message) => sanitizeMessageId(message?.extra?.shardwright?.messageIdentity?.messageId))
            .filter(Boolean)
    );
...
export async function reconcileMessageIdentityState(messages, options = {}) {
    ...
    const ssChat = ensureShardwrightChatRoot(chatMetadata);
