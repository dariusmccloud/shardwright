...
function deriveSpeakerRole(message) {
    ...
    const sourceType = String(message?.extra?.shardwright?.speakerIdentity?.sourceType || '').trim().toLowerCase();
    if (sourceType) {
        return sourceType;
    }
    return 'assistant';
...
function createBaseDetection(context) {
    ...
    const speakerEntityId = String(context.message?.extra?.shardwright?.speakerIdentity?.speakerEntityId || '').trim() || null;
