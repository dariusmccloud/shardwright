...
async function appendConflictingArchitecturalShard(root, chatFilePath) {
    ...
    secondShard.extra.shardwright.messageIdentity.messageId = makeMessageId('d4');
    secondShard.extra.shardwright.messageIdentity.revisionHash = 'sha256:rev-d4';
...
===END===`;
    ...
    header.chat_metadata.shardwright.shardManifests.push(secondManifest);
...
test('candidate run compiles admitted artifacts into isolated candidate state and emits a no-promotion report', async () => {
    ...
    assert.match(result.report.candidateRelativePath, /^shardwright\/candidates\//);
...
test('older candidate reports load without C0.5C arrays and preserve historical blockers verbatim', () => {
    ...
    fs.writeFileSync(reportPath, JSON.stringify({
        ...
        candidateRelativePath: `shardwright/candidates/${baseName}.db`,
        manifestRelativePath: `shardwright/candidates/${baseName}.manifest.json`,
        reportRelativePath: `shardwright/candidates/${baseName}.report.json`,
