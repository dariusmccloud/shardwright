...
- **Known gap (found in 3a.1 review, 2026-09-25; not yet fixed):** the `.git` match is case-sensitive. Windows filenames are not, so declaring `.GIT/HEAD` is not refused and reads the real git metadata file (confirmed by probe). Directory expansion is unaffected, because Windows reports the actual name `.git`. Proposed fix (3a.2, unauthorized): match the `.git` segment case-insensitively for both refusal and exclusion.
