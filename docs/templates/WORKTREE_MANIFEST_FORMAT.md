...
- **The `.git` match ignores case** (3a.2, authorized 2026-09-25). Any segment equal to `.git` compared without regard to letter case (`.git`, `.GIT`, `.Git`, and so on) counts as `.git`, for both declared-path refusal and expansion exclusion. Before 3a.2, the match was case-sensitive: on Windows, whose filenames ignore case, a declared `.GIT/HEAD` was not refused and read the real git metadata file (confirmed by probe during the 3a.1 review).
