...
lawful runtime work should be another read-only route over proven replay state. Stale-lock
reclamation requires a separately entered lock-recovery policy because the current runtime
contract requires an exclusive write lease but does not define lease ownership, expiry, or
safe reclamation.
