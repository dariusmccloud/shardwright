...
**Current blocker:** persisted projection rebuild, remaining routes, UI, and lock
recovery remain unimplemented. The next lawful runtime work should be a separately bounded
persisted projection rebuild slice or another read-only route over proven replay state; neither
may create new authority or repair missing ledger entries.
...
| Context Sheet membership | [Membership Link Contract](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_LINK_CONTRACT.md); [Runtime Persistence and Replay Ownership](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_RUNTIME_PERSISTENCE_AND_REPLAY_OWNERSHIP_CONTRACT.md) | `ENTERED` | NOMINATE, VALIDATE, LINK, SUCCEED, IMPACT_DECIDE, RECONCILE diagnostic result admission, and disposable current-use replay projection `PROVEN` (`tools/server-plugin/shardwright-memory/membership.js`, `membership.test.mjs`, 40/40 on 2026-08-28). The read-only current-use route is PROVEN (index.js, index.test.mjs, 24/24 on 2026-08-29). Remaining routes, persisted projections, UI, and lock recovery remain open. | Implement a bounded persisted projection rebuild or the next read-only route over proven replay state. |
