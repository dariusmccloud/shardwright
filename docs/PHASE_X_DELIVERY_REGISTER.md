...
**Current blocker:** membership `RECONCILE`, persisted projection rebuild, routes, UI, and lock
recovery remain unimplemented. The next lawful runtime work should be a separately bounded
membership `RECONCILE` diagnostic result slice or a read-only route over the proven replay
projection; neither may create new authority or repair missing ledger entries.
...
| Context Sheet membership | [Membership Link Contract](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_LINK_CONTRACT.md); [Runtime Persistence and Replay Ownership](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_RUNTIME_PERSISTENCE_AND_REPLAY_OWNERSHIP_CONTRACT.md) | `ENTERED` | NOMINATE, VALIDATE, LINK, SUCCEED, IMPACT_DECIDE, and disposable current-use replay projection `PROVEN` (`tools/server-plugin/shardwright-memory/membership.js`, `membership.test.mjs`, 35/35 on 2026-08-28). `RECONCILE`, routes, persisted projections, UI, and lock recovery remain open. | Implement bounded membership `RECONCILE` diagnostic result or a read-only route over the replay projection. |
...
- `node --test membership.test.mjs` in `tools/server-plugin/shardwright-memory/` passed 35/35 on 2026-08-28 after replay/current-use projection coverage was added. In addition to NOMINATE/VALIDATE/LINK/SUCCEED/IMPACT_DECIDE proof, it proves: replay reconstructs current-use state from LINK, SUCCEED, and IMPACT_DECIDE authority; impact replay requires exact Context Sheet Identity structural-event custody; missing structural custody fails closed without repairing membership authority; and conflicting current-use events block the affected link instead of selecting a semantic winner by write order. This proves durable admission, read-back, and disposable current-use replay only — not semantic validation, RECONCILE, routes, persisted projections, or UI.
