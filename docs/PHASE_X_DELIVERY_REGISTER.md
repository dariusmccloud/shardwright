...
| Transcript Index and Recall Experience | [Transcript Index and Recall Experience Contract](contracts/SHARDWRIGHT_TRANSCRIPT_INDEX_AND_RECALL_EXPERIENCE_CONTRACT.md) | Parallel operational-continuity track | `ENTERED` (v0.8.17); inventory through durable visibility projection and candidate/window work remain proven as recorded above. Capacity accounting is corrected and `PROVEN`: host-measured prompt capacity already includes system/card and active-chat content and excludes the host's configured reply limit, so v2 retains only a retrieval ceiling plus optional explicit safety headroom. Valid v1 profiles migrate with an unchanged legacy snapshot and zero headroom; malformed profiles fail closed. Catalog/profile/UI proof is 14/14 and server preflight 4/4 on 2026-09-07. `extension_settings.shardwright` remains the sole persisted authority. Authenticated read-only candidate, anchor-resolution, contextual-window, multi-window assembly, and explicit character-binding routes are proven; deterministic bundle presentation is `PROVEN` (11/11 focused proof) and preserves all row custody/timestamp metadata with explicit omissions. No host-default mutation, injection, reranking, dossier, or Phase X integration is claimed. |
| Operator settings catalog | [Operator Settings Catalog Contract](contracts/SHARDWRIGHT_OPERATOR_SETTINGS_CATALOG_CONTRACT.md) | Cross-cutting operator-configuration foundation | `ENTERED` (v0.2.0); Transcript Recall v2 declares two global controls—retrieval ceiling and explicit safety headroom—and its migration preserves a valid v1 snapshot without reinterpreting legacy reservations. Catalog/profile/UI proof is 14/14 on 2026-09-07. Character scope has no declared persistence owner and therefore remains unavailable; no generic renderer is claimed. |
| Host pre-dispatch context planning | [Host Pre-Dispatch Context Planning Contract](contracts/SHARDWRIGHT_HOST_PRE_DISPATCH_CONTEXT_PLANNING_CONTRACT.md), [Approved-Bundle Materialization Contract](contracts/SHARDWRIGHT_HOST_APPROVED_BUNDLE_MATERIALIZATION_CONTRACT.md), and [Transient Dispatch Diagnostics Contract](contracts/SHARDWRIGHT_HOST_TRANSIENT_DISPATCH_DIAGNOSTICS_CONTRACT.md) | Parallel operational-continuity design track | Planning is `ENTERED` (v0.4.1); host planning and binding modules are packaged under the tracked extension tree, and candidate through approved materialization are proven in the 35/35 host suite. Final orchestration remains separately governed. |
| Transcript candidate selection policy | [Transcript Candidate Selection Policy Contract](contracts/SHARDWRIGHT_TRANSCRIPT_CANDIDATE_SELECTION_POLICY_CONTRACT.md) | Orchestration prerequisite | `ENTERED` (v0.1.0); defines Continuity/Archaeology posture, FTS-first ordering, ambiguity refusal, Adequacy versus Sufficiency, operator-preference limits, and complete-bundle capacity refusal. No runtime selector or reranker is authorized yet. |
