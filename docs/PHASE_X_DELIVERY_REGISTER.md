...
**Current blocker:** persisted projection rebuild, routes, UI, and lock
recovery remain unimplemented. The next lawful runtime work should be a separately bounded
read-only route over the proven replay projection or a persisted projection rebuild slice;
neither may create new authority or repair missing ledger entries.
...
| Context Sheet membership | [Membership Link Contract](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_LINK_CONTRACT.md); [Runtime Persistence and Replay Ownership](contracts/PHASE_X_CONTEXT_SHEET_MEMBERSHIP_RUNTIME_PERSISTENCE_AND_REPLAY_OWNERSHIP_CONTRACT.md) | `ENTERED` | NOMINATE, VALIDATE, LINK, SUCCEED, IMPACT_DECIDE, RECONCILE diagnostic result admission, and disposable current-use replay projection `PROVEN` (`tools/server-plugin/shardwright-memory/membership.js`, `membership.test.mjs`, 40/40 on 2026-08-28). Routes, persisted projections, UI, and lock recovery remain open. | Implement a read-only route over the replay projection or a bounded persisted projection rebuild. |
