...
| Similharity | Keep; host a pinned copy under project control | Original repository is gone; copies with original history exist | Pinned copy preserved 2026-09-21 in `vendor/similharity/` (original-source commit `d695fbb`); dependency audit and behavioral test still open | Design review, L-2 |
...
| D-B | Hosting Similharity under project control | Chris | Provenance gate: exact commit, license text, dependency audit, API compatibility test, pinned copy | Keep the current install; do not update it. Preservation done 2026-09-21: verified bundle of the installed original-source commit `d695fbb` in `vendor/similharity/` with a provenance record. Static endpoint check: the pin and Dogoo9 3.2.1 pass; KritBlade/VectFox 3.3.4 does not define `POST /rerank`, which Shardwright's reranker client calls. Still open: dependency audit, behavioral test, update procedure |
...
| D-H | Minimum supported Node version (`node:sqlite` is a release candidate) | Chris | Host's requirement | **Recorded 2026-09-25 (Claude; pending Codex review): supported minimum Node 24.21.0**, the verified runtime. Evidence: the server-plugin suite passes 399 of 400 on Node 24.21.0; the one failure is a packaging defect unrelated to the runtime (see the Work Board). Hard floor below that: `node:sqlite` works without `--experimental-sqlite` only from Node 22.13.0 / 23.4.0 (nodejs.org/api/sqlite.html, 2026-09-25). The plugin also calls `DatabaseSync.serialize()` for managed snapshots and promotion rollback (`sqlite-node.js`, `core.js`, `promotion.js`); the Node docs list it as added in 26.1.0 with no backport, yet it exists in 24.21.0 locally, so its earliest version is unverified. **Gap:** SillyTavern 1.18.0 and SillyBunny 1.7.0 both declare `node >= 20`, so a host on Node 20 through 22.12 can run SillyTavern while the plugin cannot load. SillyBunny runs the plugin on Bun (declared `bun >= 1.3.14`); the Bun path passes the packaged Node/Bun parity tests. Declared `engines.node`: `>=24.21.0` in the plugin's `package.json` (2026-09-26). Not done here: a clear startup error on older Node; a separate slice if wanted |
