...
|---|---|---|
| **Server plugin parked in SillyBunny** | Chris moved the `shardwright-memory` plugin link (and the bridge link) out of `D:\SillyBunny\plugins\` on purpose, because the plugin was having issues; moving it rather than deleting it keeps it restorable. SillyBunny loads plugins only from `plugins\` (`src/server-main.js:502`), so the plugin is currently not loaded. **Needed from Chris:** what the issues were (errors, crashes, a feature not working), so the cause can be diagnosed. SillyBunny runs plugins on Bun, and the plugin has a separate Bun path (`sqlite-bun.js`). | Found 2026-09-25 during slice B; confirmed intentional by Chris |
| **Plugin packaging script is stale** | `tools/server-plugin/package-shardwright-memory.mjs` lists 21 static payload files; the plugin's `index.js` imports 25 local modules, and 15 of them are missing from the list (every transcript-recall route, plus `context-planning-digest-route.js`). A packaged plugin would fail to load. The plugin suite's packaging test already fails on this (399 of 400). Candidate slice: update the payload list so the packaging test passes; needs approval | Found 2026-09-25 during slice B |
...
| 11 | B: README install steps for the server plugin | Ordinary (documentation) | Claude (implements); Codex (reviews) | none | `README.md` | Same approval as entry 10 | **Blocked, not built:** truthful install steps depend on two findings (see Needs you): the packaging script is stale, and the SillyBunny plugin is intentionally parked while its issues are unexplained |
