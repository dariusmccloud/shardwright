...
|---|---|---|
| **3d containment (security)** | Approve the Codex sandbox fix (exclude the OS temp directory, `-a never`, two canaries, model-free regression test) and `agent-adapter.js` scope, then rerun the live test | Step 3 draft, "3d round 1" |
