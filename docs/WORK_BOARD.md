...
| 7 | 3b.2 (remove withdrawn `SELF_REVIEW_DEFERRED` from the ledger) | Ordinary | Codex (implements); Claude (reviews) | 3c (`1dca3af`) | Declaration in `docs/proposals/STEP3_RUNNER_SLICE_DECLARATIONS_DRAFT.md` (written after approval) | Chris, 2026-09-25, in session ("3d/e both approved + ledger cleanup"); approved before the declaration was written, so Codex reviews the declaration first | Authorized; declaration awaiting Codex's review |
| 8 | 3e (review backlog; fake agents only; cap 3) | Ordinary | Codex (implements); Claude (reviews) | 3b.2 reviewed and revalidated | Declaration (tests 18–26), written after approval | Same approval as entry 7 | Authorized; waits on 3b.2 |
| 9 | 3d (real Claude and Codex adapters; fixture repositories only) | Integration | Codex (implements); Claude (reviews) | 3e reviewed and revalidated | Declaration (offline tests, plus a live test that is opt-in and uses real usage), written after approval | Same approval as entry 7 | Authorized; waits on 3e |
