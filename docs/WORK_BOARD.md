...
|---|---|---|
| **SillyBunny plugin link location** | SillyBunny loads server plugins only from `D:\SillyBunny\plugins\` (`src/server-main.js:502`), but the `shardwright-memory` link (and a `charmemory-continuity-bridge` link, both dated 2026-09-11) sit at `D:\SillyBunny\`, so the loader never sees them. Decide whether to move the links into `plugins\` (a change on Chris's machine, not in this repository) | Found 2026-09-25 during slice B |
...
| 11 | B: README install steps for the server plugin | Ordinary (documentation) | Claude (implements); Codex (reviews) | none | `README.md` | Same approval as entry 10 | **Blocked, not built:** truthful install steps depend on two findings (see Needs you): the packaging script is stale, and SillyBunny's plugin link sits outside the folder its loader reads |
