...
- **Update 2026-09-21:** the installed plugin turned out to be a clone of the original remote at `d695fbb` (2.0.0), which is now preserved as a verified git bundle in `vendor/similharity/` (see its `PROVENANCE.md`). That is the original-source copy Chris asked for. The static endpoint check also found that the VectFox 3.3.4 plugin does not define `POST /rerank`, which Shardwright's reranker client calls.
